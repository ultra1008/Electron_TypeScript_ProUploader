...
(function ($) {
    $(document).ready(function () {
        ...
        var originMovesWithPadding = true;
        {
            ...
            $('body').append('<div id="transformableeraseme" style="overflow: hidden; position: absolute; border-style: solid; border-width: 1px; border-color: rgb(0, 0, 0); left: -1000px; top: -1000px; width: 250px; height: 250px;">\
            ...
            if (("" + torigin).match(/px/)) {
                ...
                if (parseInt(torigin) == px) {
                    originusesborder = false;
                }
            }
            ...
            if (window.getComputedStyle) {
                ...
                csleft = tmp2[0].currentStyle['left'];
            }
        ...
        jQuery.fn.transformable = function (options, args, args2) {
            ...
            return this.each(function () {
                ...
                if ($.support.matrixFilter) {
                    $.fn.transformable.defaults.sizeIcons = false;
                }
        ...
        */
        ...
        function enablerotate(tf) {
            tf.append('<div title="Rotate" class="transformable-handle-rotate transformable-handle fa fa-undo" style="right: 1px; top: 1px; position: absolute; z-index: 2000;"></div>');
        ...
        jQuery.fn.setTransform = function (options, args) {
            return this.each(function () {
                ...
                if (options == 'skewx') {
                    ...
                    transform = "matrix(" + options[0].toFixed(16) + ',' + options[1].toFixed(16) + ',' + options[2].toFixed(16) + ',' + options[3].toFixed(16) + ',' + options[4].toFixed(16) + ',' + options[5].toFixed(16) + ')';
                    ...
                    if ($.support.matrixFilter) {
                        if (posbottom != '' && posbottom != 'auto' && (postop == '' || postop == '0px')) {
                            t.css('bottom', posbottom);
                        } else {
                            t.css('top', postop);
                        }
                        if (posright != '' && posright != 'auto' && (posleft == '' || posleft == '0px')) {
                ...
                if (tt.rotate == 0 && tt.skewy == 0 && tt.skewx == 0 && tt.scaley == 1 && tt.scalex == 1) {
                    t.removeClass('istransformed');
                } else {
                    t.addClass('istransformed');
                }
                ...
                if (xfirst) {
                    options = jQuery.fn.transformable.matrix('rotate(' + tt.rotate + ') skewY(' + tt.skew[1] + ') skewX(' + tt.skew[0] + ') scaleY(' + tt.scaley + ') scaleX(' + tt.scalex + ')');
                } else {
                    options = jQuery.fn.transformable.matrix('rotate(' + tt.rotate + ') skewX(' + tt.skew[0] + ') skewY(' + tt.skew[1] + ') scaleY(' + tt.scaley + ') scaleX(' + tt.scalex + ')');
                }
                transform = "matrix(" + options[0].toFixed(16) + ',' + options[1].toFixed(16) + ',' + options[2].toFixed(16) + ',' + options[3].toFixed(16) + ',' + options[4].toFixed(16) + ',' + options[5].toFixed(16) + ')';
                t.css('transform', transform);
                t.attr('data-transform', transform);
        ...
        function scmousedown(e) {
            ...
            if (transform.scale[0] < 0) {
                tobj.startWidth = -tobj.startWidth;
            }
            ...
            if (transform.scale[1] < 0) {
                tobj.startHeight = -tobj.startHeight;
            }
        ...
        function smousedown(e) {
            ...
            tobj = {};
            var h = $(this);
            if (h.hasClass('transformable-handle-skew-v')) {
                tobj.vert = 1;
            } else {
                tobj.vert = 0;
            }
        ...
        function smousemove(e) {
            ...
            while (tobj.angle.rad < -Math.PI) {
                tobj.angle.rad += twopi;
            }
            while (tobj.angle.rad >= Math.PI) {
                tobj.angle.rad -= twopi;
            }
            ...
            while (tobj.angle.deg > 180) {
                tobj.angle.deg -= 360.0;
            }
            while (tobj.angle.deg < -180) {
                tobj.angle.deg += 360.0;
            }
            var retx = true, rety = true;
            ret = true;
        ...
        function rmousemove(e) {
            ...
            if (!(ret === false)) {
                tobj.self.setTransform('rotate', tobj.angle.rad);
            } else {
                return false;
            }
        ...
        jQuery.ui.resizable.prototype._mouseStart = function (event) {
            if (!this.element.data('transformable-opts')) {
                return orig_resize_mousestart.call(this, event);
            }
        ...
        jQuery.ui.resizable.prototype._mouseDrag = function (event) {
            if (!this.element.data('transformable-opts')) {
                return orig_resize_drag.call(this, event);
            }

            fixevent(event);
            var w, h, c, points, smp = this.originalMousePosition, contained = this.element.data('parentcontained');
            ...
            c = this.element.data('startoff');
            points = [{ x: smp.left, y: smp.top }];
            ...
            if (c && c.transformed) {
                points = $.singleLevelTP(points, { x: event.pageX, y: event.pageY }, imm(c.totalmatrix));
            }
        ...
        jQuery.fn.transformable.resize = function (e, ui) {
            ...
            if (!iscontained(t, t.parent(), t.data('parentoffset'))) {
                ...
                t.removeData('resizeoutofbounds');
            }
        };
        jQuery.fn.transformable.dragstart = function (e, ui) {
            if (!ui) {
                return false;
            } //somehow this function is getting triggered on a resizable with transformable set and no draggable
            e.stopPropagation();
            fixevent(e);
            var t = $(this);
            t.tDragStart(e.pageX, e.pageY, false);
        };
        ...
        $.fn.tDragStart = function (x, y, skipuntransformed, hasoffset) {
            ...
            self.each(function (i) {
                ...
                if (container[0] && container[0] == t[0]) {
                    container = t.parent();
                }
                if (container == 'parent') {
                    container = t.parent();
                ...
                if (typeof (c) != 'array' && (container == t.parent()[0] || container[0] == t.parent()[0])) {
                    container = $(container);
                    c = container.tOffset();
                    //console.log("parentoff => " + JSON.stringify(c));
                    t.data('parentoff', c);
                    var pcn = c.corners;
                    //the dim of the box being constrained
                    var boxdim = t.tOffset();
                    //console.log("boxdim    => " + JSON.stringify(boxdim));
                    ...
                    //console.log("pcn   => " + JSON.stringify(pcn));
                    //console.log("boxcn => " + JSON.stringify(boxcn));
                    ...
                    var bl = borderWidth(container, 'left');
                    var br = borderWidth(container, 'right');
                    var bt = borderWidth(container, 'top');
                    var bb = borderWidth(container, 'bottom');

                    var range = {};
                    range.tl = { x: lc, y: tc };
                    range.parenttl = pcn[0];

                    range.invertx = (lc < pcn[0].x) || (rc > pcn[1].x);
                    range.inverty = (tc < pcn[0].y) || (bc > pcn[3].y);

                    range.l = lc - (pcn[0].x + bl);;
                    range.r = (pcn[1].x - br) - rc;

                    range.t = tc - (pcn[0].y + bt);
                    range.b = (pcn[3].y - bb) - bc;
        ...
        var orig_draggable_drag = jQuery.ui.draggable.prototype._mouseDrag;
        function tMouseDrag(event, noPropagation) {
            if (!this.element.data('transformable-opts')) {
                return orig_draggable_drag.call(this, event);
            }
            ...
            if (!noPropagation) {
                ...
                if (($(ui.helper).data("ui-draggable") && !$(ui.helper).data("ui-draggable").scrollParent) || this._trigger('drag', event, ui) === false) {
                    this._mouseUp({});
                    return false;
                }
        ...
        function tsnapstart(event, ui) {
            var el = $(this);
            var i = el.data("ui-draggable"), o = i.options;
            i.snapElements = [];
            var inst = el.data("ui-draggable");
            var toff = el.tOffset();
        ...
        function tsnapdrag(event, ui) {
            ...
            var inst = el.data("ui-draggable"), o = inst.options;
            ...
            for (var i = inst.snapElements.length - 1; i >= 0; i--) {
                ...
                if (o.snapMode != 'inner') {
                    ...
                    if (ts) {
                        el.data('snaptop', t - el.data('tHeight'));
                    }
                    if (bs) {
                        el.data('snaptop', b);
                    }
                    if (ls) {
                        el.data('snapleft', l - el.data('tWidth'));
                    }
                    if (rs) {
                        el.data('snapleft', r);
                    }
                ...
                if (o.snapMode != 'outer') {
                    ...
                    if (bs) {
                        el.data('snaptop', b - el.data('tHeight'));
                    }
                    if (ls) {
                        el.data('snapleft', l);
                    }
                    if (rs) {
                        el.data('snapleft', r - el.data('tWidth'));
                    }
                }
        ...
        */
        ...
        $.fn.tDrag = function (x1, y1, ui) {
            ...
            self.each(function (i) {
                ...
                if (t.hasClass('ui-draggable-dragging') && !t.data("ui-draggable").containment) {
                    range = false;
                }
                var delta;
                if (range) {
                    ...
                    if (range.invertx) {
                        //console.log("invertx");
                        if ((delta.x + range.l) > 0) {
                            //console.log("left limit");
                            x = ustartm.x - range.l;
                        }
                        if ((delta.x + -range.r) < 0) {
                    ...
                    if (range.inverty) {
                        //console.log("inverty");
                        if ((delta.y + range.t) > 0) {
                            //console.log("top limit");
                            y = ustartm.y - range.t;
                        }
                        if ((delta.y + -range.b) < 0) {
        ...
        // requires t, but can be sped up if you have toff,p,and/or poff
            ...
            if (options && typeof (options) == 'object' && (options.left || options.top)) {
                ...
                return this.each(function () {
                    ...
                    if (options.left === undefined) {
                        options.left = toff.left;
                    }
                    if (options.top === undefined) {
                        options.top = toff.top;
                    }
                ...
                var innercorners;
                if (options === true) {
                    var bw = { left: 0, top: 0 }; //t.borderWidth('all');
                ...
                */
                ...
                if (tm[0] == 1 && tm[1] == 0 && tm[2] == 0 && tm[3] == 1 && tm[4] == 0 && tm[5] == 0) {
                    off.transformed = false;
                } else {
                    off.transformed = true;
                }
                //element will get this class if it is transformed, without regart to parents
                if (sm[0] == 1 && sm[1] == 0 && sm[2] == 0 && sm[3] == 1 && sm[4] == 0 && sm[5] == 0) {
                    t.removeClass('istransformed');
                } else {
                    t.addClass('istransformed');
                }
            ...
            if (usespx) {
                ...
                if (o && o.length == 3) {
                    ...
                    if (originMovesWithScrollBar) {
                        ...
                        if (scr.y) {
                            dh += originMovesWithScrollBar;
                        }
                        if (scr.x) {
                            dw += originMovesWithScrollBar;
                        }
                    }
        ...
        jQuery.singleLevelTP = function (points, center, transform) {
            ...
            if (typeof (transform) != 'object') {
                m = jQuery.fn.transformable.matrix(transform);
            } //a,b,c,d,x,y
            else {
                m = transform;
            }
        ...
        jQuery.fn.matrixToArray = function (returnFalse) {
            ...
            if (transform != undefined && transform != 'none' && transform.replace(/\s+/g, '') != 'matrix(1,0,0,1,0,0)' && transform != '') {
                var m = transform.split('matrix(')[1].split(')')[0].split(',');
                if (m) {
